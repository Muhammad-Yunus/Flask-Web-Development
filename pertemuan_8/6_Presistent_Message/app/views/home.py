...
class SensorDataFactory :
  ...
  def insertData(self):
    # generate dummy data
    self.data =  {
      'temperature' : np.around(np.random.uniform(low=21, high=40), decimals=2),
      'humidity'    : np.around(np.random.uniform(low=0, high=100), decimals=2),
      'time'        : datetime.now()
    }

    # save to db for presistent
    Temperature = Sensor(name='Sensor Node 001',
          device_no='A150001',
          device_type='Temperature',
          time=self.data['time'],
          value=self.data['temperature']
          )
    Humidity = Sensor(name='Sensor Node 001',
          device_no='A150001',
